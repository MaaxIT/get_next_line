...
{
	static char	*rmd[4096];
	char		*buf;
	char		*tmp;

	buf = NULL;
	if (fd >= 0 && BUFFER_SIZE > 0 && rmd[fd])
	{
		tmp = check_remaind(&rmd[fd]);
		if (tmp)
			return (tmp);
		else
		{
			if (ft_strlen(rmd[fd]) > 0)
			{
				buf = ft_strndup(rmd[fd], ft_strlen(rmd[fd]));
				if (!buf)
					return (NULL);
			}
			free(rmd[fd]);
	...
	if (BUFFER_SIZE > 0 && fd >= 0 && loop_buf(fd, &buf, &rmd[fd]) == 1)
		return (NULL);
	return (buf);
}
